...
import { DEVNET_CONNECTION } from '../utils/constants';

@Injectable()
export class DevnetSeedService {
  private readonly logger = new Logger(DevnetSeedService.name);
  ...
  constructor(rpcManager: RpcManagerService) {
    this.connection = rpcManager.getConnection(DEVNET_CONNECTION);
  }
  ...
  public async seedDatabase() {
    ...
    try {
      ...
      this.logger.error(`Unable to seed database... ${err}`);
    } finally {
